class PhotosController < ApplicationController
  before_action :current_user_must_be_photo_latest_owner, only: %i[edit update destroy]
  ...
  # GET /photos
  def index
    @q = current_user.latest_photos.ransack(params[:q])
    @photos = @q.result(distinct: true).includes(:owner, :latest_owner, :likes, :comments, :commenters, :fans).page(params[:page]).per(10)
  ...
  def photo_params
    params.require(:photo).permit(:caption, :image, :location, :owner_id, :rating, :status)
  end
end
