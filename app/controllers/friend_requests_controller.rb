class FriendRequestsController < ApplicationController
  before_action :current_user_must_be_friend_request_pending_sender, only: %i[edit update destroy]

  before_action :current_user_must_be_friend_request_recipient, only: %i[edit update destroy]
  ...
  def index
    ...
    @friend_requests = @q.result(distinct: true).includes(:sender, :recipient, :follower, :pending_sender).page(params[:page]).per(10)
  end
  ...
  def current_user_must_be_friend_request_pending_sender
    set_friend_request
    unless current_user == @friend_request.pending_sender
      redirect_back fallback_location: root_path, alert: "You are not authorized for that."
    end
  end

  def current_user_must_be_friend_request_recipient
    set_friend_request
